...
public class LinkedBinaryTree<T> implements BinaryTreeADT<T>
{
	...
	protected void setRootNode(BinaryTreeNode<T> node){
		root=node;
	}

	/* Add 1 to the tree's size */
	...
	/* Subtract 1 from the tree's size */
	...
	{
		ArrayUnorderedList tempList1 = new ArrayUnorderedList();
		inorder(root,tempList1);
		return tempList1.toString();
	}
	...
	public Iterator<T> iteratorLevelOrder() 
	{
		...
		BinaryTreeNode<T> tempRoot=new BinaryTreeNode<T>(root.getElement());
		nodes.enqueue(root);
		while (!nodes.isEmpty()){
			tempRoot = (BinaryTreeNode<T>)nodes.dequeue();
			if (tempRoot != null){
				temp.addToRear(tempRoot.getElement());
				temp.addToRear(tempRoot.getLeft().getElement());
				temp.addToRear(tempRoot.getRight().getElement());
			}
		}
